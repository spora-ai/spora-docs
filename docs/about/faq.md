...
CLI: `php bin/spora plugin:install vendor/package`. Web UI: **Plugins → Browse → Install** (requires `SPORA_PLUGIN_INSTALL_ENABLED=true`). External path: set `SPORA_PLUGINS_PATHS=/path/to/checkout` and `PluginLoader` picks up the manifest on the next request. See [Install API](/develop/plugins/install-api) for the full contract.
