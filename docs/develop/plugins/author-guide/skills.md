...
The same rule applies to every multi-operation tool — these tools declare an `action` discriminator field because they ship more than one `#[ToolOperation]`: `time` (`now`, `format`) — both ops return a `weekday` (long English name, ISO 8601 Monday-based), `skill` (`read`, `files`), `agent` (`read_notes`, `write_notes`, …), `user_info` (`get_health_data`, `get_locations`, …), `media` (`upload`, `retrieve`, `list`, …). The prose shortcut `<tool>.<operation>` is fine in headings for readability, but worked examples in code blocks must show the JSON-call shape (`tool(action: "op", param: "value")`).
