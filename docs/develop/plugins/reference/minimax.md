...
| `minimax:music`    | `music-3.0`          | Operations: `compose` (instrumental or with `lyrics`, 1-3500 chars), `write_lyrics` (full song from a topic), `edit_lyrics` (rewrite existing lyrics). `music-2.6` and `music-cover` are also accepted by the upstream endpoint. **Deprecated since Aug 20 2026 for new users** per MiniMax's Music API Service Adjustment Notice — the paid APIs are no longer available to new users; existing paying users can keep using this tool. New deployments should prefer [MiniMax Audio](https://www.minimax.io/audio) or self-host [MiniMax Music 3 on Hugging Face](https://huggingface.co/MiniMaxAI/MiniMax-Music3). The `minimax-music-legacy` skill surfaces the deprecation context to the agent. |
...
- `minimax-music-legacy` — same wire shape as `minimax-music`, surfacing the Aug 20 2026 deprecation **Deprecated since Aug 20 2026 for new users** and the recommended alternatives. Load only when the operator is an existing paying user and explicitly wants the legacy API path; new deployments should not load this skill.
