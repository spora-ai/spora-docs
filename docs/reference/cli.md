...
| `db:seed`                    | Run the bootstrap seeder on an existing install: creates the admin user + sample agent if missing. Insert-only — never patches an existing admin row.                                          | `app/Console/Commands/SeedCommand.php`                  |
