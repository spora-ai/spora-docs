...
| `spora:setup`                | Run migrations and seed a fresh database. Existing installs are skipped — use `db:seed` (bootstrap) or `db:repair-admin` (promote an existing row) for repairs. Used by the Docker entrypoint.                                                                                                    | `app/Console/Commands/SetupCommand.php`                 |
...
| `db:seed`                    | Run the bootstrap seeder on an existing install: creates the admin user + sample agent if missing (insert-only — never patches an existing admin row), and reconciles mail templates against `email-templates/` (inserts missing YAMLs, preserves operator-customised rows; `--force` is opt-in). | `app/Console/Commands/SeedCommand.php`                  |
| `db:repair-admin [email]`    | Promote an existing user to `verified=1`, `Role::ADMIN`, `status=1`. Idempotent, refuses to create a missing row (operator-only). Default email: `admin@spora.local`.                                                                                                                             | `app/Console/Commands/RepairAdminCommand.php`           |
