...
| `TOOLS_RECOMMENDS_SKILLS_MISSING` | 500         | `error`   | One or more `#[Tool(recommendsSkills: ...)]` entries do not resolve on disk under any scanner root. Strict-mode — no env-flag opt-out. `error.details.violations[]` carries the offenders as `{ tool_class, tool_name, missing[] }`. See [Concepts → Tool system → `/api/v1/tools` errors](/reference/concepts/tools#errors). |
