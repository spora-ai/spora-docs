...
User-supplied filenames flowing into `media_assets.filename` are scrubbed by `Utf8Sanitizer::scrubString` at the `MediaIngestRequest` construction site, and the serializer still sanitises on read as defense-in-depth. See [UTF-8 sanitizer](/reference/concepts/utf8-sanitizer).
