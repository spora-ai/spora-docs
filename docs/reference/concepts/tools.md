...
| `get_source`        | no              | required         | Read the source of an asset so the LLM can iterate on it (re-typeset a `.typ` source, re-ingest an extracted document). Text-shaped mimes (text/*, JSON, XML, YAML, SVG, CSV, x-typst) return the bytes inline, capped at `GET_SOURCE_TEXT_MAX` (5 MiB). Binary mimes do **not** return raw bytes — instead the asset's extracted `markdown_content` is surfaced (truncated to `GET_MEDIA_MARKDOWN_PREVIEW_BYTES`, 8 KiB) when the operator populated it during ingestion. When no `markdown_content` exists, fail with a hint pointing at `get_media` and `list_derivatives`. External assets (`storage_mode=external`) have no Spora-side payload — fail with a `get_media` hint. Approval-gated because the LLM is exfiltrating source content. |
