...
| `app_env`                     | `SPORA_APP_ENV`                     | `development`                         | Mandatory in production. Silences PHP deprecation warnings and removes the `debug` envelope from `/api/*` JSON errors.                                                                                                                                                                                   |
...
| `mercure_publish_url`         | `SPORA_MERCURE_PUBLISH_URL`         | falls back to `mercure_url`           | Publisher endpoint. In Docker, `http://localhost/...` here triggers Caddy's auto-HTTPS redirect when `SERVER_NAME` is a public domain — use the service name (`http://spora:80/...`) or omit the variable. |
...
| `log_path`                    | `SPORA_LOG_PATH`                    | `storage/spora.log`                   | Docker deployments should set `stdout` so records stream to the container's log driver. The framework default `storage/spora.log` is the env-less LAMP/FTP fallback.                                                                                                                                    |
