...
| ----------------------------- | ----------------------------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `app_url`                     | `SPORA_APP_URL`                     | auto-detected                         | Public URL of the instance (used for email links). See [Environment variables](/start/operators/env-vars#application) for the resolution order and the no-`X-Forwarded-*` trust guarantee.                               |
| `app_prefix`                  | `SPORA_APP_PREFIX`                  | `''` (empty)                          | Path prefix Spora is mounted under, e.g. `/spora` when reverse-proxied under a sub-path. Normalised: leading/trailing slashes stripped, bare `/` collapses to empty. Used for verification / password-reset email links. |
