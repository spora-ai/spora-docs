...
| `worker_runtime_mode`         | `SPORA_WORKER_RUNTIME_MODE`         | package default (`server` for `spora-ai/spora`, `client` for `spora-ai/spora-shared`) | Runtime mode. `server` = daemon drains the queue. `client` = browser `SharedWorker` ticks tasks the user ran. See [Deployment modes](/reference/concepts/deployment-modes).                                                                                                                                                                                                                                                                                                                               |
| `tick_lease_seconds`          | `SPORA_TICK_LEASE_SECONDS`          | `600`                                                                                 | Lease TTL (seconds) for in-flight ticks. Extended on every step boundary inside `Orchestrator::tick()`.                                                                                                                                                                                                                                                                                                                                                                                                   |
| `worker_stale_minutes`        | `SPORA_WORKER_STALE_MINUTES`        | `60`                                                                                  | Minutes before a `RUNNING` task with an expired lease is reaped.                                                                                                                                                                                                                                                                                                                                                                                                                                          |
