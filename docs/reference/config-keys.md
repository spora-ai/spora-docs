...
| `mercure_publish_url`         | `SPORA_MERCURE_PUBLISH_URL`         | falls back to `mercure_url`           | Publisher endpoint. In Docker with a public `SERVER_NAME`, `http://localhost/...` here triggers Caddy's auto-HTTPS redirect — use the service name (`http://spora:80/...`) or omit. |
