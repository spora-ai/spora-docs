...
If the chat's subtle running indicator (`Step N of M` + Abort button) stays put for longer than you expected — or the agent keeps calling the same tool without converging — the first thing to try is the **Abort** button next to the step counter. The loop halts at the next natural break point (after the in-flight tool returns, between LLM turns) and the chat flips to an **Aborted — paused by you** banner (with a small **manual** badge). Click **Resume** for a default _"continue"_ prompt, or type a redirect (e.g. "stop iterating, just give me the summary") in the composer and press Enter — the task resumes with the new prompt.
