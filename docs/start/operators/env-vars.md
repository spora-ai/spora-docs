...
| -------------------------- | ------------- | -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SPORA_APP_URL`            | auto-detected | `app_url`            | Full public URL of this instance, including non-standard port. Used for verification / password-reset email links. Resolution order — first wins: `SPORA_APP_URL` env var, then the request's `HTTP_HOST`, then the web server's `SERVER_NAME` (Apache `ServerName`), then `http://localhost`. **Never reads `X-Forwarded-*` headers** — those are spoofable and Spora has no trusted-proxy allowlist. Operators behind a reverse proxy that rewrites `Host` MUST set this. |
| `SPORA_APP_PREFIX`         | `''`          | `app_prefix`         | Path prefix Spora is mounted under, e.g. `/spora` when reverse-proxied under a sub-path. Leading/trailing slashes are stripped; bare `/` collapses to empty. Prepended to email-link paths (verification, password reset).                                                                                                                                                                                                                                                  |
