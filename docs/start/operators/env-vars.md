...
| `SPORA_APP_PREFIX`         | `/spora`      | `app_prefix`         | Path prefix Spora is mounted under. **Default is `/spora`** because the admin UI ships under `public/spora/` and plugins ship under `public/plugins/<name>/`. Operators running Spora at the host root (e.g. when developing their own frontend) MUST set `SPORA_APP_PREFIX=""` to opt out. Leading/trailing slashes are stripped; bare `/` collapses to empty. Prepended to email-link paths (verification, password reset).                                               |
