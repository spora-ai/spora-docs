...
| `SPORA_TICK_LEASE_SECONDS`   | `600`    | `tick_lease_seconds`   | **Client-mode only.** Lease TTL (seconds) for in-flight ticks. Written to `lease_owner` + `lease_expires_at` by `TaskTickController::tick` and `ScheduledRunProcessor::dispatchAndTick`. Extended at every orchestrator step boundary so a healthy tick never lapses. Server-mode daemon paths skip the lease entirely (rely on `updated_at`), so this var is read but not consulted in server mode. The reaper flips a client-mode row to `FAILED` with `error_code: WORKER_DISCONNECTED` once the lease expires + `SPORA_WORKER_STALE_MINUTES` elapses with no progress. |
| `SPORA_WORKER_STALE_MINUTES` | `60`     | `worker_stale_minutes` | Grace period (minutes) before an orphan `RUNNING` task is reaped. In client mode this applies after the lease has expired; in server mode it applies directly to `updated_at` (no lease was granted). Should exceed worst-case LLM round-trip time. Set to `0` to disable the reaper.                                                                                                                                                                                                                                                                                      |
...
| `SPORA_MERCURE_LEGACY_USER_TOPIC` | `false`                      | `mercure_legacy_user_topic` | Plan B fan-out rollout flag. When `true`, the publisher also emits task events to the legacy `user/{userId}/tasks` topics for every visible user, in addition to the principal-keyed `principal/{principalId}/tasks` topics. Set to `true` if you deploy the spora-core backend before the spora-frontend cutover (frontend versions older than v0.15.0 still subscribe to the legacy topics). Will be removed in the release after the frontend cutover. |

When all three primary vars are unset, `MercurePublisher::publish()` early-returns and the frontend falls back to polling. See [Agent loop and async mode](/reference/concepts/agent-loop-async) for the SSE topic model.
